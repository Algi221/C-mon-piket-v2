...
import { Menu, X, Calendar, LayoutDashboard, Award, Users, BookOpen, Settings, Home, LogOut, ShieldAlert } from 'lucide-react';
...
export default function Navbar({ currentTab, setCurrentTab, supabaseConnected, currentUser, onLogout }: NavbarProps) {
  ...
  const menuItems = [
    ...
    { id: 'dashboard', name: 'BERANDA PIKET', icon: LayoutDashboard, color: 'bg-cyan-300', roles: ['siswa', 'guru'] },
    { id: 'schedule', name: 'JADWAL PIKET', icon: Calendar, color: 'bg-pink-300', roles: ['guest', 'siswa', 'guru'] },
    { id: 'attendance', name: 'ABSENSI & LOG', icon: BookOpen, color: 'bg-green-300', roles: ['siswa', 'guru'] },
    { id: 'leaderboard', name: 'LEADERBOARD', icon: Award, color: 'bg-purple-300', roles: ['guest', 'siswa', 'guru'] },
    { id: 'students', name: 'KELOLA SISWA', icon: Users, color: 'bg-orange-300', roles: ['guru'] },
    { id: 'supabase', name: 'CONFIG DATA', icon: Settings, color: 'bg-zinc-400', roles: ['guru'] },
  ];
      ...
      <header className="sticky top-0 z-40 w-full border-b-4 border-black bg-white px-4 py-3 font-sans md:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <nav className="hidden items-center gap-3 lg:flex">
            ...
            {currentUser && (
              <div className="ml-4 flex items-center gap-2 border-l-2 border-black pl-4">
                <div className="text-right">
                  <p className="text-[10px] font-black text-black leading-none uppercase truncate max-w-28">{currentUser.name}</p>
                  <span className="text-[8px] font-black bg-yellow-300 border border-black px-1.5 py-0.2 uppercase mt-0.5 inline-block">
                    {currentUser.role}
                  </span>
                </div>
                <button
                  onClick={onLogout}
                  title="Keluar"
                  className="flex h-10 w-10 items-center justify-center border-2 border-black bg-red-400 text-black shadow-[2px_2px_0px_0px_#000000] hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[3px_3px_0px_0px_#000000] active:translate-x-0 active:translate-y-0 active:shadow-none cursor-pointer"
                >
            ...
            {/* Supabase Indicator */}
            <div className={`ml-2 h-3.5 w-3.5 rounded-full border-2 border-black shadow-[1px_1px_0px_0px_#000000] ${supabaseConnected ? 'bg-green-400' : 'bg-red-400'}`} title={supabaseConnected ? 'Supabase Terhubung' : 'Offline Mode (LocalStorage)'} />
          </nav>
