...
import { BookOpen, Search, Calendar, Check, X, ExternalLink } from 'lucide-react';
    ...
    <div className="mx-auto max-w-6xl p-4 font-sans md:p-6 animate-fade-in">
      ...
      <div className="mb-8 border-4 border-black bg-pink-300 p-6 shadow-[6px_6px_0px_0px_#000000]">
        ...
        <p className="mt-2 text-sm font-bold text-zinc-800 uppercase">
          Daftar riwayat laporan piket kelas beserta status verifikasi kehadiran siswa!
        </p>
      </div>
      ...
      <div className="mb-8 border-4 border-black bg-white p-4 shadow-[4px_4px_0px_0px_#000000] flex flex-col md:flex-row items-center gap-4">
        <div className="relative w-full flex-1">
          ...
          <input
            type="text"
            placeholder="CARI NAMA SISWA ATAU TANGGAL (YYYY-MM-DD)..."
            value={searchTerm}
      ...
      {filteredLogs.length === 0 ? (
        <div className="border-4 border-dashed border-zinc-300 bg-white p-12 text-center shadow-[4px_4px_0px_0px_#000000]">
          ...
          <p className="text-xs font-bold text-zinc-400 uppercase mt-1">Belum ada laporan piket terkirim atau hasil pencarian tidak cocok.</p>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredLogs.map((log) => {
            ...
            const statusColors = {
              verified: 'bg-green-300 border-green-700 text-black',
              pending: 'bg-yellow-300 border-yellow-700 text-black',
              rejected: 'bg-red-300 border-red-700 text-red-900',
            };
              ...
              >
                <div>
                  ...
                  <div className="border-b-2 border-black p-3 bg-zinc-50 flex items-center justify-between">
                    ...
                    <span className={`border-2 border-black text-[9px] font-black px-2 py-0.5 shadow-[1px_1px_0px_0px_#000000] uppercase ${statusColors[log.status] || 'bg-zinc-300'}`}>
                      {log.status === 'verified' ? 'DISETUJUI' : log.status === 'pending' ? 'MENUNGGU' : 'DITOLAK'}
                    </span>
                  </div>
                  ...
                  {/* Body Content */}
                  <div className="p-4">
                    <h3 className="text-lg font-black text-black leading-none uppercase truncate">
                      Oleh: {log.reporter_name}
                    </h3>
                    <p className="text-[9px] font-bold text-zinc-400 uppercase mt-1">Pukul: {formatTime(log.created_at)} WIB</p>
                    ...
                    {/* Detailed presence status list */}
                    {log.details && log.details.length > 0 && (
                      <div className="mt-4">
                        <p className="text-[9px] font-black uppercase text-zinc-400 mb-1.5">Kehadiran Petugas:</p>
                        <div className="flex flex-wrap gap-1.5">
                          {log.details.map((det) => (
                            <span
                              key={det.id}
                              className={`flex items-center gap-1 border px-1.5 py-0.5 text-[9px] font-black uppercase ${
                                det.is_present === 1 
                                  ? 'bg-green-50 border-green-600 text-green-700' 
                                  : 'bg-red-50 border-red-600 text-red-700'
                              }`}
                            >
                              {det.is_present === 1 ? <Check className="h-2.5 w-2.5 stroke-[4px]" /> : <X className="h-2.5 w-2.5 stroke-[4px]" />}
                              {det.student_name}
                            </span>
                          ))}
                        </div>
                ...
                <div className="p-3 border-t border-black bg-zinc-50 text-[9px] font-black uppercase text-zinc-400 text-center">
                  LOG ID: {log.id}
                </div>
              </div>
            );
