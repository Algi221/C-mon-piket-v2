...
interface LeaderboardProps {
  students: User[]; // The raw students list
}

export default function Leaderboard({ students }: LeaderboardProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [leaderboardData, setLeaderboardData] = useState<(User & { points: number; streak: number })[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const calculateLeaderboard = async () => {
      try {
        const sortedData = await db.getLeaderboard();
  ...
  const filteredStudents = leaderboardData.filter((s) =>
    s.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const topThree = leaderboardData.slice(0, 3);
  const remaining = filteredStudents.slice(filteredStudents.length > 0 ? (leaderboardData.findIndex(s => s.id === filteredStudents[0].id) < 3 ? 3 : 0) : 0);
    ...
    <div className="mx-auto max-w-4xl p-4 font-sans md:p-6 animate-fade-in">
      ...
      <div className="mb-8 border-4 border-black bg-green-300 p-6 shadow-[6px_6px_0px_0px_#000000]">
        ...
        <p className="mt-2 text-sm font-bold text-zinc-800 uppercase">
          Siapakah pahlawan kebersihan kelas terbaik? Poin dihitung dinamis (+15 poin) berdasarkan kehadiran pada piket yang sudah diverifikasi oleh Guru!
        </p>
      </div>

      {loading ? (
        <div className="border-4 border-black bg-white p-8 text-center text-xs font-black uppercase shadow-[4px_4px_0px_0px_#000000]">
          Mengalkulasi Skor Juara Kelas...
        </div>
      ) : (
        <>
          {/* Podium of Top 3 */}
          {leaderboardData.length > 0 && searchTerm === '' && (
                ...
                {topThree[1] && (
                  <div className="w-full md:w-48 flex flex-col items-center order-2 md:order-1 mt-6">
                    <div className="border-4 border-black bg-cyan-100 p-4 shadow-[4px_4px_0px_0px_#000000] text-center w-full relative">
                      ...
                      <h3 className="font-black text-black truncate uppercase text-xs mt-1 leading-none">{topThree[1].name}</h3>
                      <p className="text-[10px] font-black text-zinc-500 uppercase mt-2">{topThree[1].points} Poin</p>
                      <div className="flex items-center justify-center gap-1 mt-2 text-[10px] font-black text-orange-600">
                        <Flame className="h-4.5 w-4.5 fill-orange-500 stroke-[2px]" />
                ...
                {topThree[0] && (
                  <div className="w-full md:w-56 flex flex-col items-center order-1 md:order-2">
                    <div className="border-4 border-black bg-yellow-100 p-5 shadow-[4px_4px_0px_0px_#000000] text-center w-full relative border-b-4">
                      ...
                      <h3 className="font-black text-black truncate uppercase text-sm mt-1 leading-none">{topThree[0].name}</h3>
                      <p className="text-xs font-black text-zinc-600 uppercase mt-2">{topThree[0].points} Poin</p>
                      <div className="flex items-center justify-center gap-1 mt-2 text-[10px] font-black text-red-600">
                ...
                {topThree[2] && (
                  <div className="w-full md:w-44 flex flex-col items-center order-3 mt-12">
                    <div className="border-4 border-black bg-pink-100 p-4 shadow-[4px_4px_0px_0px_#000000] text-center w-full relative">
                      <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-3xl">🥉</span>
                      <h3 className="font-black text-black truncate uppercase text-xs mt-1 leading-none">{topThree[2].name}</h3>
                      <p className="text-[10px] font-black text-zinc-500 uppercase mt-2">{topThree[2].points} Poin</p>
                      <div className="flex items-center justify-center gap-1 mt-2 text-[10px] font-black text-orange-600">
          ...
          <div className="border-4 border-black bg-white shadow-[6px_6px_0px_0px_#000000]">
            ...
            {filteredStudents.length === 0 ? (
              ...
              filteredStudents.map((s, idx) => {
                const actualRank = leaderboardData.findIndex((st) => st.id === s.id) + 1;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-3 text-left pl-2">
                      <p className="font-black text-xs text-black leading-none truncate">{s.name}</p>
                      <p className="text-[8px] text-zinc-400 font-bold mt-1">NIPD: {s.nipd}</p>
                    </div>
                    ...
                    <div className="col-span-2 hidden sm:flex items-center justify-center gap-1 font-black text-red-600">
                      {s.streak > 0 ? (
                        ...
                        <span className="text-zinc-400 font-bold text-[9px]">KOSONG</span>
                      )}
                    </div>
                  </div>
                );
